    ...
    where TEntity : class, IEntity<TId>, new()
{
    ...
    /// <param name="ct">A <see cref="CancellationToken"/> that is automatically populated by the ASP.NET Core framework from the incoming HTTP request.</param>
    ...
    /// When the result is truncated, the response includes an <c>X-Total-Count</c> header with the true total count.
    /// </remarks>
    Task<TEntity[]> GetAll(CancellationToken ct = default);
    ...
    Task<PagedList<TEntity>> GetPaged(int? pageSize, int? pageNumber, string? filter, string? sort, string? graph, CancellationToken ct = default);
    ...
    /// <exception cref="Models.Exceptions.ObjectNotFoundNeedleCrudException">Thrown when an entity with the specified <paramref name="id"/> is not found.</exception>
    Task<TEntity> GetGraph(TId id, string graph, CancellationToken ct = default);
}